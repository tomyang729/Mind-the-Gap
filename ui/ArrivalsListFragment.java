...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        ...
        //Iterator<ArrivalBoard> arrivalBoardIterator = selected.iterator();
        for(ArrivalBoard ab : selected) {
            if(ab.getTravelDirn().equals(travelDirn) && ab.getLine().getId().equals(lineId)) {
                for(Arrival next : ab) {
                    arrivals.add(next);
                }
            }
